...
function generateCaptcha(params) {
    ...
    if (params.color != undefined)
        options.background = params.color;
...
app.get('/captcha', (req, res) => {
    ...
    var captcha = generateCaptcha({ text: text, color: color, font: font, width: width, height: height, font_size: font_size, noise: noise });
    if (captcha === undefined)
        return res.status(500).json({
            error: 'Requête mal formatée, le champ text n\'est pas présent.',
            indication: 'Seul le paramètre text est obligatoire, les autres sont optionnels. Pour la couleur, c\'est soit en hexa, soit les noms anglais',
            example_url: 'Veuillez utiliser des paramètres de la forme text=blabla&color=blue&font=comicsansms.ttf&width=800&height=400&font_size=100&noise=1'
        });
